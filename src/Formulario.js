...
export default function Formulario({ user, onLogout }) {
  ...
  const imprimirPDF = () => {
    ...
    html2pdf().set({ margin:10, filename:\`protocolo-\${protocolo}.pdf\`, jsPDF:{unit:"mm",format:"a4"} }).from(html).save()
  }
  ...
  return (
    <div className="relative max-w-xl mx-auto mt-10 p-6 bg-white shadow rounded">
      ...
      <p className="text-sm text-gray-600 italic mb-4">🛡️ Mensagens 100% confidenciais.</p>

      {!enviado && (
        <div className="mb-4 text-yellow-700 bg-yellow-50 border border-yellow-200 p-3 rounded text-sm">
          ⚠️ <strong>RASCUNHO:</strong> ainda <strong>não enviado</strong>. Sem validade jurídica.
        </div>
      )}

      {enviado && (
        <div className="mb-4 text-green-700 bg-green-50 border border-green-200 p-3 rounded text-sm">
          ✅ <strong>ENVIADO.</strong> Protocolo: <strong className="font-mono">{protocolo}</strong><br/>
          <div className="flex gap-3 mt-2">
            <button onClick={imprimirPDF} className="underline text-sm text-blue-700">Imprimir PDF</button>
            <button onClick={() => { setEnviado(false); setMensagem(""); setSetorDestino("") }} className="underline text-sm text-gray-600">Nova mensagem</button>
          </div>
        </div>
      )}
      ...
      {!enviado && (
        <form onSubmit={handleEnviar} className="flex flex-col gap-3">
          ...
          <label className="font-semibold text-sm">Setor destinatário</label>
          <select className="p-2 border rounded" value={setorDestino} onChange={e => setSetorDestino(e.target.value)} required>
            <option value="">Selecione o setor</option>
            {SETORES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <label className="font-semibold text-sm">Mensagem</label>
          <textarea className="p-2 border rounded min-h-[120px]" placeholder="Digite sua mensagem..."
            value={mensagem} onChange={e => setMensagem(e.target.value)} required />
          <div className="grid grid-cols-2 gap-2">
            <div><label className="font-semibold text-sm">Nome</label>
              <input className="w-full p-2 border rounded bg-gray-50 text-sm" value={user.nome_completo} disabled /></div>
            <div><label className="font-semibold text-sm">Setor</label>
              <input className="w-full p-2 border rounded bg-gray-50 text-sm" value={user.setor} disabled /></div>
          </div>
